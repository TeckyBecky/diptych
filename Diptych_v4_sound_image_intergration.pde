...
lBOne = loadImage("Button-A.png");
  ...
  rBFour = loadImage("Button-Down.png");
  
  //rBThree = loadImage("rBThree.png");
  rBThree = loadImage("Button-Right.png");
...
{
  if (key=='a') {
    lOne = loadImage("100Emoji.png");
    image(lOne, 80, 50);
    lOneSound.play();
  }
  if (key=='s') {
    lTwo =  loadImage("BasketballEmoji.png");
    image(lTwo, 80, 50);
    // stop all sound
    lTwoSound.play();
  }
  if (key=='d') {
    lThree =  loadImage("NerdEmoji.png");
    image(lThree, 80, 50);
    //lThreeSound.play();
  }
  if (key=='f') {
    lFour =  loadImage("PhoneEmoji.png");
    image(lFour, 80, 50);
    //lFourSound.play();
  }
  if (key=='v') {
    lFive =  loadImage("ThumbsUpEmoji.png");
    image(lFive, 80, 50);
    //lFiveSound.play();
  }
  if (key == ';' || key == ':') {    // rBOne = ;
    rOne = loadImage("BreifCaseEmoji.png");
    image(rOne, 460, 50);
    //rOneSound.play();
  }
  if (key == 'l' || key == 'L') {    // rBTwo = l/L
    rTwo = loadImage("FlexEmoji.png");
    image(rTwo, 460, 50);
    //rTwoSound.play();
  }
  
  if (key == 'k' || key == 'K') {    // rBThree = k/K
    rThree = loadImage("HourGlassEmoji.png");
    image(rThree, 460, 50);
    //rThreeSound.play();
  }
  if (key == 'j' || key == 'J') {    // rBFour = j/J
    rFour = loadImage("PantsEmoji.png");
    image(rFour, 460, 50);
    //rFourSound.play();
  }
  if (key == 'n' || key == 'N') {    // rBFive = n/N
    rFive = loadImage("StarEmoji.png");
    image(rFive, 460, 50);
    //rFiveSound.play();
  }
}
